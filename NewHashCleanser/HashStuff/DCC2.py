...
class DCC2(Hash):
    ...
    # this function will go through a directory and parse all the information from all the files
        ...
        for file in fileslist:
            ...
            if os.path.isfile(file_path):
                hashes = DCC2.Secrets_Parser(file_path, pattern)
                hashes2 = hashes + hashes2
            if (mode == 'DCC2'):
                DCC2.List_Cleanser(hashes2, 'secretdumps.DCC2')
            else:
                hashes2 = DCC2.Remove_Duplicates(hashes2)
                DCC2.list_to_file(hashes2, 'secretdumps.NTLM')
    ...
    # Function to go through secrets folders and pull hashes
        ...
        if (os.path.exists(input) == True):
            ...
            # Pull the hashes
            hashes = DCC2.Pattern_Parser(content, pattern) 
        return hashes
    ...
    # This function will take the file of cracked passwords from hashcat and the NTLM files and match the username to the password
        if (os.path.isfile(hashes) == True and os.path.isfile(cracked) == True):
            ...
            with open(hashes) as file:
                ...
                hashes_from_file = DCC2.Remove_Unwanted_Chars(hashes_from_file)
                nonCracked_hash, nonCracked_username = DCC2.Match_Parser(hashes_from_file, pattern_HASH)
                file.close()
            print('Grabbing cracked passwords')
            with open(cracked) as file:
                cracked_from_file = file.readlines()
                cracked_from_file = DCC2.Remove_Unwanted_Chars(cracked_from_file)
                isCracked_hash, isCracked_password = DCC2.Match_Parser(cracked_from_file, pattern_CRACKED)
                file.close()
        else:
            print('Cracked hash or Hash file does not exist, please check the filename')
        DCC2.Match_Helper(isCracked_hash, isCracked_password, nonCracked_hash, nonCracked_username)
        ...
        for y in range (len(nonCracked_hash)):
            for x in range (len(isCracked_hash)):
                if (isCracked_hash[x] == nonCracked_hash[y]):
                    ...
                    DCC2.userList.append(crackedAccount)
        DCC2.User_List_To_File()
    ...
    #Function to go through a file of DCC2 hashes and clean them up
        ...
        if (os.path.exists(input) == True):
            ...
            DCC2.List_Cleanser(hash_list, 'tmp')
            # closing the file
            file.close()
    ...
    # Function to clean a list of dcc2 hashes
        ...
        cleaned_hashes = DCC2.Remove_Duplicates(cleaned_hashes)
        # send the list to the 'list_to_file' function
        DCC2.list_to_file(cleaned_hashes, input)
    ...
    def User_List_To_File():
        ...
        for x in range (len(DCC2.userList)):
            file.write(DCC2.userList[x].username+"   ")
            file.write(DCC2.userList[x].hash+"   ")
            file.write(DCC2.userList[x].password + "\n")
        file.close()
